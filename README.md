...
v1.3.0
 ...
 - 完成了所有的取模工作，现在所有图片和汉字可以在单片机上显示了
 
 - 存在的问题
     -  超绝钝感上位机拥有令人十分着急的识别能力
     
     - 目前数字图片均为128x128左右大小，因为过大的图片导致了单片机内存不足，然而较小尺寸的图片甚至使得上位机的识别能力更加捉急
  
     - 由于🐸汉化组的失误（采用了错误的工具vscode）导致了无法修复的汉字乱码（锟斤拷伟锟斤拷、锟斤拷锟斤拷和锟斤拷旻！）——🔨汉化组，我们需要你的帮助！
